...
inline void ImGui::FileBrowser::Display()
{
    ...
    for (auto& sec : pwd_)
    {
        ...
        if (secIdx > 0)
            SameLine();
        if (SmallButton((const char*)sec.u8string().c_str()))
                        ...
                        (!rsc.isDir && !(flags_ & ImGuiFileBrowserFlags_SelectDirectory)))
                    {
                        ...
                        if (!(flags_ & ImGuiFileBrowserFlags_SelectDirectory))
                        {
                            ...
                            strcpy_s(inputNameBuf_->data(), inputNameBuf_->size(),
                                (const char*)selectedFilename_.u8string().c_str());
#else
    ...
    if (Button(cancel_.c_str()) || closeFlag_ || ((flags_ & ImGuiFileBrowserFlags_CloseOnEsc) && IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) && escIdx >= 0 && IsKeyPressed(ImGuiKey_Escape)))
        CloseCurrentPopup();
...
inline void ImGui::FileBrowser::SetPwdUncatched(const std::filesystem::path& pwd)
{
    ...
    for (auto& p : std::filesystem::directory_iterator(pwd))
    {
        ...
        rcd.showName = (rcd.isDir ? directoryLabel_ : fileLabel_) + rcd.showName + p.path().filename().string();
